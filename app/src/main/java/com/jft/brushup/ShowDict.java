...
public class ShowDict extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i < fullwords.size(); ++i) {
            words.add(fullwords.get(i).getWord());
        ...
        listView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
                ...
                try {
                    activity.putExtra("showWord",words.get(mTemporaryList.get(position)));
                    activity.putExtra("showMeaning",fullwords.get(mTemporaryList.get(position)).getMeaning());
                    startActivity(activity);
                } catch (Exception e) {}
            }
        });
